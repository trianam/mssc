...
\item $[p|(q|r)=(p|q)|r]$: La dimostrazione avviene mostrando che
  ...
  transizioni.
\item $[p|nil=p]$: Si dimostra con il fatto che $R$ \`e chiusa per
...
\item $[p \setminus L = p\quad\text{ se }\mathcal{L}(p) \cap (L \cup
  \overline{L}) = \emptyset]$: Se $p\xrightarrow{\mu}p'$ allora per la
  regola $(RES)$ della semantica $p\setminus
  L\xrightarrow{\mu}p'\setminus L$ solo se $\mu,\overline{\mu}\not\in
  ...
  quindi vale sempre che $p\setminus L\xrightarrow{\mu}p'\setminus L$,
  e la tesi vale perch\'e $<p'\setminus L,\ p'>\in R$ e dalla
