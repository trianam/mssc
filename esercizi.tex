...
dimostrata perch\'e vale $E\xRightarrow{s}$ per l'assioma $(Star_1)$.
\paragraph{Caso base $|s|=1$} se la traccia \`e di lunghezza uno,
allora essa pu\`o essere $s=a$ o $s=b$, senza perdita di generalit\`a
si assume il primo caso.

...

\paragraph{Passo induttivo $|s|>0$} si ha che la traccia pu\`o essere
...
\begin{align*}
  ...
  &1  
\end{align*}
Nell'ultimo caso si ha che necessariamente $s'=\varepsilon$ e quindi si
riconduce al caso base perch\'e $s$ \`e di lunghezza 1. In tutti gli
altri casi si ha che $X\ \xRightarrow{\varepsilon}\ F$ \`e l'unica
computazione possibile, quindi $F\
\xRightarrow{s'}\ 1$ e usando l'ipotesi induttiva vale che $E\
\xRightarrow{s'}\ 1$. La tesi si ottiene giustificando che esiste $E\
\xRightarrow{a}\ E$ e quindi $E\ \xRightarrow{as'=s}\ 1$. Si
giustificano i passi $E\ \xrightarrow{a}\ 1;E\
\xrightarrow{\varepsilon}\ E$ con:
\begin{align*}
